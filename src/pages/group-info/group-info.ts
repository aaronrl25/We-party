...
export class GroupInfoPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public dataProvider: DataProvider,
    ...
    public alertProvider: AlertProvider, public angularfireDatabase: AngularFireDatabase, public imageProvider: ImageProvider, public camera: Camera) { }
        ...
        },
        {
          ...
          handler: data => {
            ...
            this.dataProvider.getGroup(this.groupId).update({
              ...
              this.angularfireDatabase.object('/accounts/' + firebase.auth().currentUser.uid + '/groups/' + this.groupId).remove().then(() => {
          ...
          handler: data => {
            ...
            group.messages.forEach((message) => {
              if (message.type == 'image') {
                console.log("Delete: " + message.url + " of " + this.group.$key);
                this.imageProvider.deleteGroupImageFile(this.group.$key, message.url);
              }
            });
            // Delete group image.
            console.log("Delete: " + group.img);
            this.imageProvider.deleteImageFile(group.img);
            console.log(firebase.auth().currentUser.uid + " " + this.group.$key);
            this.angularfireDatabase.object('/accounts/' + firebase.auth().currentUser.uid + '/groups/' + this.group.$key).remove().then(() => {
              this.dataProvider.getGroup(this.group.$key).remove();
            });
          }
        }
      ]
