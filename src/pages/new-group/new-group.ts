...
export class NewGroupPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public imageProvider: ImageProvider, public dataProvider: DataProvider, public formBuilder: FormBuilder,
    public alertProvider: AlertProvider, public alertCtrl: AlertController, public angularfireDatabase: AngularFireDatabase, public app: App, public loadingProvider: LoadingProvider, public camera: Camera) {
    ...
    this.groupForm = formBuilder.group({
      ...
      ubicacion: Validator.grouptUbicacionValidator,
      date:Validator.groupDateValidator,
      hrs:Validator.groupHoraValidator
  ...
  // Proceed with group creation.
    ...
    this.group.ubicacion = this.groupForm.value["ubicacion"];
    this.group.date = this.groupForm.value["date"];
    this.group.hrs = this.groupForm.value["hrs"];

    // Add group to database.
    this.angularfireDatabase.list('groups').push(this.group).then((success) => {
      let groupId = success.key;
      // Add group reference to users.
      this.angularfireDatabase.object('/accounts/' + this.groupMembers[0].$key + '/groups/' + groupId).update({
        messagesRead: 1
      });
      for (var i = 1; i < this.groupMembers.length; i++) {
        this.angularfireDatabase.object('/accounts/' + this.groupMembers[i].$key + '/groups/' + groupId).update({
          messagesRead: 0
        });
      }
